...
> background execution & run status — merged here; the async path is captured in Epic 14.)*
...
### 5.3 — SSRF guard on fetch · `lane:L3-data-platform` · *deferred → 12.6*
...
story **12.6**. `labels: user-story, area:security` · **depends on:** 5.2.
...
- The full text is **not** stored inline on the Cosmos doc (2 MB item cap + RU cost on every read); the
  Cosmos `ResultItem` (Epic 9) carries the **reference + `EvalRationale`**, so the reasoning and the exact
...
> metric + golden-set recall harness) is **deferred** and folds into Epic 12's evals/observability.
...
### 6.4 — Verdict-distribution metric + recall check on golden set · `lane:L3-data-platform` · *deferred → 12.1 / 12.3*
...
harness in the repo yet (verdict routing only *logs* counts today). Tracked to land with the Epic 12
eval suite + observability dashboards (12.1 / 12.3).
...
is deferred to Epic 12 as a nice-to-have. (See implementation-plan.md Phase 6.)
...
## ✅ Epic 7 — Per-step MAF executor decomposition (mid-pass checkpointing) · `phase-7` · *L1-led* · **Complete**
> The topic-group loop body was decomposed into **seven MAF executors** wired by edges, enabling
> **between-step checkpointing** (resume mid-pass instead of replaying a whole pass) and per-document
> fan-out. Delivered ahead of the fan-out work (Epic 13) because it shares the same decomposition.
> *(Delivered on the `phase-7` branch / PR #36.)*

### 7.1 — Decompose topic-group pass into per-step executors (mid-pass checkpointing + intra-group fan-out) · `lane:L1-orchestration` · **✅ Done**
...
`labels: user-story, area:maf, needs-design` · **depends on:** 2.1, 2.2, 6.2.

**Epic demo:** each pipeline step is its own MAF executor; a mid-pass failure resumes after the last completed step instead of replaying the whole pass; the loop-back/finalize branch is driven by the Loop Controller executor's response.

---

## ✅ Epic 8 — Enrichment + Categorize (Impact Area + Tags) + Summary + Company View · `phase-8` · *L2-led* · **Complete**
The Company View and its objective fields are produced **per vetted document after** the Agentic RAG loop
(**Option A** — revised from the original group-level design; see
`docs/company-view-per-doc-implementation-plan.md`), using the **vetted full text** of each carried
document. **Categorization (Impact Area + Tags) is per document:** for each carried `ResultItem`, one
Impact Area call + one Tags call are grounded on that document's own full text and — together with the
Company View — stored on that document's `CompanyViewRecord` (`ResultItem.CompanyView`). There is **no**
group-level aggregate record. (Enrichment, if ever enabled, remains per item.)
...
### 8.2 — Impact Area Agent (per document) · `lane:L2-agents` · **✅ Done**
**AC:** assign **one impact area** to **each vetted document** from the **approved impact-area vocabulary** (single-label, closed set), in a **single call over that document's vetted full text**. The chosen area is set on the document's `CompanyViewRecord` (8.5), stored on `ResultItem.CompanyView`. Vocabulary is **loaded from Cosmos at runtime** (RegDocs, `doc_type = "ImpactAreas"`) — not hardcoded.
...
### 8.3 — Tags Agent (per document) · `lane:L2-agents` · **✅ Done**
**AC:** assign **one or more tags** to **each vetted document** from the **approved tag vocabulary** (multi-label), in a **single call over that document's vetted full text**. **Separate agent / LLM call** from 8.2 (one task per prompt; multi-label; independently tunable). The selected tags are set on the document's `CompanyViewRecord` (8.5), stored on `ResultItem.CompanyView`. Vocabulary is **loaded from Cosmos at runtime** (RegDocs, `doc_type = "tags"`) — not hardcoded.
...
**AC:** a **professional, neutral summarization** of the regulatory update — *what changed* — grounded in the **vetted full text** of each document. **There is no separate summary agent:** this is produced by the **Company View agent (8.5)** in the **same single LLM call** that produces the document's Company View, as the record's `Summary of Update` field. The summary uses **no** historical Company Views (the prior-view exemplars steer the Company View only).
`labels: user-story, area:llm` · **depends on:** 6.2.

### 8.5 — Company View Agent (per vetted document; RAG over prior views by jurisdiction) · `lane:L2-agents` · **✅ Done**
**AC:** produce **one Company View per vetted document** — a single record grounded on that document's **vetted full text**, plus its **own** Impact Area and Tags from 8.2/8.3, via RAG over prior Company Views retrieved **by jurisdiction** (jurisdiction **from the scan request**). The output is a **structured record shaped to match the `RegulatoryUpdatesCsv` columns** (`CompanyViewRecord`: Jurisdiction · Impact Area · Tags · Title of Update · Summary of Update · Company View · Level of Authority · Status of Change · Announcement date · Effective Date of Change · Supporting reference · Regulator), stored on **`ResultItem.CompanyView`** — one per document, **not** a group field. Objective fields are set deterministically — **jurisdiction** from the scan request, and **dates + supporting reference** from the document — while **Impact Area + Tags come from the per-document 8.2/8.3 agents**; the judgement fields (title, **Summary of Update**, Company View, level of authority, status, regulator) are **synthesized by the model** in a **single Structured-Outputs call** over the document's **full text**. The **Summary of Update** is a neutral account of *what changed*, grounded only in the full text and using **no** prior views; the **Company View** is additionally steered (house style/tone) by the prior views retrieved by jurisdiction. It runs **once per vetted document, after** the per-item enrichment; the jurisdiction-scoped prior views are fetched **once per group** by the finalize step and passed into each document's call. Local testing uses a **CSV** of historical records; production source is **SQL** (relational by jurisdiction) — SQL wiring deferred behind a retrieval abstraction.
...
**Open items:** the **Regulator** field lives on each document's `CompanyViewRecord` (synthesized by the model) so the record matches the `RegulatoryUpdatesCsv` shape; the unused `ResultItem.Regulator` / `ImpactArea` / `Tags` placeholders were **removed** in the per-document change (Option A); **jurisdiction** is on the scan-request contract (`ScanRequest.Jurisdiction`, flowed via `RunContext.Jurisdiction`) — **done**; CSV→SQL retrieval swap is a later task; the enumerated lists in 8.2/8.3 are the **reference copy** — the runtime source is **Cosmos RegDocs** (re-seed rather than edit prompts when the vocabulary changes).

**Epic demo:** each carried document has its **own** Company View record (shaped like `RegulatoryUpdatesCsv`, on `ResultItem.CompanyView`) carrying its impact area, tags, a neutral `Summary of Update`, and the `Company View` — produced in a single call grounded in that document's vetted **full text** and prior jurisdiction-matched views (CSV locally); Enrichment remains parked.
...
`labels: user-story, area:cosmos` · **depends on:** 9.1, 2.1.
...
`labels: user-story, area:storage` · **depends on:** 9.2, 5.1 · *(merged: publish view + export)*
...
`labels: user-story, area:llm` · **depends on:** 11.1, 3.3, 6.1.
...
### 12.5 — Loop-feedback Option (B): structured `SearchDirective` steer · `lane:L2-agents` · *nice-to-have*
...
### 12.6 — SSRF guard on fetch (full) · `lane:L3-data-platform` · *nice-to-have*
...
or as part of the 12.4 security review. `labels: user-story, area:security` · **depends on:** 5.2.
...
- **14.6** Async execution mode — background run + run-status store + `GET /runs/{runId}` polling + cancellation (merged out of the old Epic 2). Pull only if synchronous scans outgrow gateway timeouts (~230s on App Service) or a live-progress UI is needed (pairs with 14.3). · `lane:L1-orchestration` · `future`
...
4. **In parallel L3** takes **0.5/0.7**, then **Epic 5 storage (5.1)** and **Epic 9** prep.
