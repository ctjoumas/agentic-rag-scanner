...
flowchart TB
  ...
  subgraph WF["3. MAF Workflow — runs once per topic group, in parallel (shared throttle for OpenAI/Bing limits)"]
    ...
    CAT --> SUM["14. Company View & Summary Agent — Stage 3<br/>one per-group record from the vetted full text: neutral summary + Company View<br/>RAG over prior Company Views (by jurisdiction) steers the view only"]
  end
...
| 13 | Categorize (Stage 2) — **per document** | Two separate LLM calls **per vetted document** (Option A): one picks a single **Impact Area** (single-label, closed vocabulary), one selects **Tags** (multi-label), each grounded on that document's own full text. Results are stored on the document's `CompanyViewRecord` (`ResultItem.CompanyView`); vocabularies load from Cosmos RegDocs. **Regulator** is synthesized on that same per-document record in step 14. |
| 14 | Company View & Summary Agent (Stage 3) | One record **per vetted document**, built from that document's vetted full text in a **single call**: a neutral summary of *what changed* AND the Company View advice. RAG over prior Company Views (retrieved by jurisdiction, fetched once per group) steers the Company View only; the summary uses no history. |
