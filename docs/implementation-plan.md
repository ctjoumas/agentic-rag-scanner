...
| 13 | **Categorize (Impact Area + Tags)** | **Per document:** one Impact Area call (single-label) + one Tags call (multi-label) per vetted document, each over that document's full text; results stored on the document's `CompanyViewRecord` (`ResultItem.CompanyView`). | each carried `ResultItem` + its full text ? per-document Impact Area + Tags | **8** |
| 14 | **Company View & Summary Agent** | One record **per vetted document** from that document's **full text**, in a single call: a neutral summary of *what changed* AND the Company View advice. Prior Company Views (by jurisdiction, fetched once per group) steer the Company View only; the summary uses no history. | each carried `ResultItem` + its full text (+ prior views by jurisdiction) ? `ResultItem.CompanyView` (summary + view) | **8** |
...
**Goal:** finish the downstream agents that run **after** the Agentic RAG loop for a topic group: optional per-item enrichment, **per-document** categorization split into **Impact Area** + **Tags** (separate LLM calls, each over that document's full text), and a **Company View** agent that — in a **single full-text call** per document — produces **both** a professional **Summary of Update** and the practitioner-style **Company View**. Impact Area + Tags are stored on each document's `CompanyViewRecord` (`ResultItem.CompanyView`). **Revised to per-document (Option A) — see `docs/company-view-per-doc-implementation-plan.md`.** `? arch steps 12–14`
...
- [x] **(L2)** **Impact Area Agent (per document):** assign **one impact area** to **each vetted document** in a **single call over that document's vetted full text**, against the **approved impact-area vocabulary loaded from Cosmos** (RegDocs, `doc_type = "ImpactAreas"`). Single-label, closed set — the agent must pick exactly one; the result is stored on the document's `CompanyViewRecord` (`ResultItem.CompanyView`).
- [x] **(L2)** **Tags Agent (per document)** (separate agent / separate LLM call): assign **one or more tags** to **each vetted document** over that document's full text, from the **approved tag vocabulary loaded from Cosmos** (RegDocs, `doc_type = "tags"`). Kept separate from Impact Area on purpose — one LLM task per prompt, independently tunable/evaluable, and tags are multi-label while impact area is single-label; the result is stored on the document's `CompanyViewRecord` (`ResultItem.CompanyView`).
...
- [x] **(L2)** **Summary (merged into the Company View agent):** a **professional, neutral summarization** of the regulatory update — *what changed* — grounded in each document's **vetted full text**. **No separate summary agent:** the Company View agent produces it as the record's `Summary of Update` in the **same single call** (using no historical views).
- [x] **(L2)** **Company View Agent** (separate agent): produce **one Company View per vetted document** — a single record grounded on that document's **vetted full text**, plus its **own** Impact Area and Tags — into practitioner-style client advice, using **RAG over prior Company Views** retrieved **by jurisdiction** (jurisdiction is **passed in on the scan request**, fetched **once per group** by the finalize step and passed into each document's call). It runs **once per vetted document, after** the per-item enrichment, and its output is a **structured record shaped to match the customer's `RegulatoryUpdatesCsv`** (`CompanyViewRecord`: Jurisdiction · Impact Area · Tags · Title of Update · Summary of Update · Company View · Level of Authority · Status of Change · Announcement date · Effective Date of Change · Supporting reference · Regulator), stored on **`ResultItem.CompanyView`** (one per document). Objective fields are set deterministically — **jurisdiction** from the scan request, **dates + supporting reference** from the document, and **Impact Area + Tags from the per-document agents** — while the judgement fields (title, **Summary of Update**, Company View, level of authority, status, regulator) are **synthesized by the model** in a **single Structured-Outputs call** over the document's **full text** — the **Summary of Update** uses **no** prior views, while the **Company View** is steered (house style/tone) by them. **Retrieval source:** local testing reads a **CSV** of historical records (customer spreadsheet); the production source is **SQL** (relational by jurisdiction, e.g. `WHERE jurisdiction = 'United Kingdom'`) — SQL wiring **deferred**, behind a retrieval abstraction.
