...
| 13 | **Categorize (Impact Area + Tags)** | **Group-level:** one Impact Area call (single-label) + one Tags call (multi-label), each over **all** the group's carried updates' full text; results set on the aggregate `DeloitteViewRecord`, not per item. | carried `ResultItem`s + full text ? group Impact Area + Tags | **8** |
...
- **Jurisdiction** — **done:** added to the **scan-request contract** (`ScanRequest.Jurisdiction`, flowed via `RunContext.Jurisdiction`); the Deloitte View retrieval keys on it.
