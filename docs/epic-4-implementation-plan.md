...
> with the solution's single **Web Search agent** — a **hosted Foundry agent** carrying the
> **Grounding with Bing Custom Search** tool, scoped to the primary-source allowlist and **referenced by
> the MAF workflow** — then add the real **deterministic pre-filter** (cross-group dedupe + URL validity).
>
...
- The per-group loop in [`TopicGroupPipeline.RunPassAsync`](../AgenticRagScannerApi.Workflows/Pipeline/TopicGroupPipeline.cs)
  runs `QuerySynthesis → BingSearch → PreFilter → Fetch&Clean → RelevanceEval → LoopController`.
...
| Package | Why | Notes |
|---------|-----|-------|
| `Microsoft.Agents.AI.Foundry` | `AIProjectClient.AsAIAgent(...)` → `FoundryAgent`; lets MAF reference the hosted agent | Align with the repo's `Microsoft.Agents.AI` **1.10.0** line (latest stable today is `1.5.0`; a matching/newer build may be required — verify on restore). |
...
### Two ways to create the hosted agent definition (pick one)

1. **Portal-first (recommended for the POC):** create the Web Search agent in the Foundry portal, attach
   the **Grounding with Bing Custom Search** tool, select the connection + configuration instance, and
   reference it by name/version from code. Fewer moving parts; the allowlist lives entirely in Foundry.
2. **Code-first (reproducible):** create/version the agent via
   `AIProjectClient.AgentAdministrationClient.CreateAgentVersion(agentName, new(DeclarativeAgentDefinition{ … }))`
   with a `bing_custom_search` tool whose `search_configurations[]` carry `connection_id` +
   `instance_name` (+ optional `count`, `market`, `set_lang`, `freshness`). Better for IaC later (Epic 13.2).
...
ProjectsAgentRecord record = await project.AgentAdministrationClient.GetAgentAsync(options.AgentName);
FoundryAgent webSearch = project.AsAIAgent(record);   // standard AIAgent → RunAsync(...)
```
...
- [ ] Add **`WebSearchOptions`** under `AgenticRagScannerApi.Workflows/Configuration/` (section `"WebSearch"`),
  ...
  | `AgentName` *(Required)* | Hosted Web Search agent name. |
  | `AgentVersion` *(optional)* | Pin a version; omit to resolve latest. |
  | `MaxResults` *(Range)* | Cap `SearchHit`s returned per query (token/cost control). |
  | `Count` / `Market` / `SetLang` / `Freshness` *(optional)* | Forwarded to the tool config **only** if you create the agent code-first. |
...
The pipeline calls `IBingSearchTool.SearchAsync(query, run, ct) → IReadOnlyList<SearchHit>`. **Keep this
seam** so [`TopicGroupPipeline`](../AgenticRagScannerApi.Workflows/Pipeline/TopicGroupPipeline.cs) is
unchanged. Rename for clarity is optional.
...
- [ ] **Resolve the `FoundryAgent` once** (singleton/lazy) from `AIProjectClient.AsAIAgent(GetAgentAsync(name))`
  — agent metadata is stable for the run; don't fetch per query.
- [ ] Implement **`BingGroundingWebSearchAgent : IBingSearchTool`** (new file in `Workflows/Tools/`,
...
- [ ] **DI swap** in [`AddWorkflowServices`](../AgenticRagScannerApi/Extensions/ServiceCollectionExtensions.cs):
  `services.AddSingleton<IBingSearchTool, BingGroundingWebSearchAgent>();` (remove the stub registration).
...
- [ ] **Contract test** for `IBingSearchTool` against a fake `AIAgent`/`AIProjectClient` that returns a
...
- [ ] Extend [`TopicGroupPipelineTests`](../tests/AgenticRagScannerApi.Tests/TopicGroupPipelineTests.cs) so the
...
| Area | File(s) | Change |
|------|---------|--------|
| Web Search agent | `Workflows/Tools/BingGroundingWebSearchAgent.cs` *(new)* | Real `IBingSearchTool` over a hosted `FoundryAgent`. |
...
| DI | [`Api/Extensions/ServiceCollectionExtensions.cs`](../AgenticRagScannerApi/Extensions/ServiceCollectionExtensions.cs) | Register `AIProjectClient`, the `FoundryAgent`, swap `IBingSearchTool`, bind `WebSearchOptions`. |
...
2. **PR-2 (L2):** add `Azure.AI.Projects` + `Microsoft.Agents.AI.Foundry`, register `AIProjectClient`,
   `WebSearchOptions`, and resolve the hosted `FoundryAgent`.
3. **PR-3 (L2):** `BingGroundingWebSearchAgent : IBingSearchTool` + DI swap + allowlist verification + tests (story **4.1**).
