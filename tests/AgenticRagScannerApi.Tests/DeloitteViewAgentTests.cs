...
/// <summary>
/// Story 8.5 - the real Deloitte View MAF agent over a fake <see cref="IChatClient"/> (no network). It
/// produces ONE aggregate <see cref="DeloitteViewRecord"/> per topic group: the dates and supporting
...
/// Deloitte View, etc.) come from the model via Structured Outputs, steered by prior records retrieved by
/// jurisdiction. Covers grounding, exemplar capping, the empty-group short-circuit, and safe degradation.
/// </summary>
public class DeloitteViewAgentTests
{
    private const string ModelJson =
        """{"titleOfUpdate":"NIC and fuel-rate changes","summaryOfUpdate":"Several employer-tax updates.","deloitteView":"Employers should update payroll systems.","levelOfAuthority":"Regulator guidance","statusOfChange":"In force","regulator":"HMRC"}""";
    ...
    public async Task GenerateAsync_ReturnsNull_WhenNoItems()
    {
        ...
        var agent = new DeloitteViewAgent(chat, new FakeSource(Prior("p")), NullLogger<DeloitteViewAgent>.Instance);
    ...
    public async Task GenerateAsync_ProducesAggregateRecord_WithModelJudgementFields()
    {
        ...
        record!.DeloitteView.Should().Be("Employers should update payroll systems.");
    ...
    {
        var chat = new FakeChatClient(ModelJson);
        var agent = new DeloitteViewAgent(chat, new FakeSource([]), NullLogger<DeloitteViewAgent>.Instance);
        var items = new[]
        {
            Item("https://gov.uk/a"),
            Item("https://gov.uk/b"),
    ...
    public async Task GenerateAsync_RetrievesPriorRecords_ByRunJurisdiction()
    {
        ...
        var agent = new DeloitteViewAgent(chat, retriever, NullLogger<DeloitteViewAgent>.Instance);
    ...
    {
        var chat = new FakeChatClient(ModelJson);
        var agent = new DeloitteViewAgent(chat, new FakeSource(Prior("HOUSE-STYLE-EXEMPLAR advice text")), NullLogger<DeloitteViewAgent>.Instance);
    ...
    public async Task GenerateAsync_CapsExemplarsAtFive()
    {
        var many = Enumerable.Range(1, 9)
            .Select(i => new DeloitteViewRecord { DeloitteView = $"View number {i}" })
            .ToList();
        var chat = new FakeChatClient(ModelJson);
        var agent = new DeloitteViewAgent(chat, new FakeSource(many), NullLogger<DeloitteViewAgent>.Instance);
    ...
    public async Task GenerateAsync_KeepsAggregatedFields_WhenModelCallFails()
    {
        ...
        record.Should().NotBeNull();
        record!.DeloitteView.Should().BeNull();
        record.ImpactArea.Should().Be("Employment taxes rates & thresholds");
        record.Tags.Should().Contain("National Insurance");
        record.SupportingReference.Should().Contain("https://gov.uk/a");
    }

    private static List<DeloitteViewRecord> Prior(string view) =>
        [new DeloitteViewRecord { TitleOfUpdate = "Prior title", ImpactArea = "Impact area", SummaryOfUpdate = "Prior summary", DeloitteView = view }];

    /// <summary>Deterministic <see cref="IPriorDeloitteViewSource"/> returning a fixed list and recording the jurisdiction asked for.</summary>
    private sealed class FakeSource : IPriorDeloitteViewSource
    {
        private readonly IReadOnlyList<DeloitteViewRecord> _records;

        public FakeSource(IReadOnlyList<DeloitteViewRecord> records) => _records = records;
