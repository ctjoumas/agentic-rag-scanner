...
/// <summary>
/// CSV-backed <see cref="IPriorDeloitteViewSource"/> (Epic 8, story 8.5): reads the customer's historical
/// regulatory-updates spreadsheet (exported to CSV, path from <see cref="RegulatoryUpdatesCsvOptions"/>)
/// and returns the prior Deloitte Views for a jurisdiction. This is the local-testing source; production
...
/// </summary>
public sealed class CsvPriorDeloitteViewSource : IPriorDeloitteViewSource
{
    private readonly RegulatoryUpdatesCsvOptions _options;
    private readonly ILogger<CsvPriorDeloitteViewSource> _logger;

    private readonly object _gate = new();
    private IReadOnlyDictionary<string, IReadOnlyList<DeloitteViewRecord>>? _byJurisdiction;

    public CsvPriorDeloitteViewSource(IOptions<RegulatoryUpdatesCsvOptions> options, ILogger<CsvPriorDeloitteViewSource> logger)
    {
        _options = options.Value;
        _logger = logger;
    }

    public Task<IReadOnlyList<DeloitteViewRecord>> GetByJurisdictionAsync(string jurisdiction, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("No prior Deloitte Views found for jurisdiction '{Jurisdiction}' in the CSV source.", jurisdiction);
        return Task.FromResult<IReadOnlyList<DeloitteViewRecord>>([]);
    }

    /// <summary>Loads and caches the CSV, grouped by jurisdiction (case-insensitive keys). Thread-safe.</summary>
    private IReadOnlyDictionary<string, IReadOnlyList<DeloitteViewRecord>> GetOrLoadIndex()
    {
    ...
    private IReadOnlyDictionary<string, IReadOnlyList<DeloitteViewRecord>> Load()
    {
        if (string.IsNullOrWhiteSpace(_options.FilePath))
        {
            _logger.LogWarning("RegulatoryUpdatesCsv:FilePath is not configured; the Deloitte View agent will run without prior-view exemplars.");
            return Empty();
        }

        if (!File.Exists(_options.FilePath))
        {
            _logger.LogWarning("Deloitte Views CSV not found at '{FilePath}'; the Deloitte View agent will run without prior-view exemplars.", _options.FilePath);
            return Empty();
        }
        ...
        try
        {
            ...
            var index = new Dictionary<string, List<DeloitteViewRecord>>(StringComparer.OrdinalIgnoreCase);
            var total = 0;

            foreach (var row in csv.GetRecords<CsvRow>())
            {
                var jurisdiction = row.Jurisdiction?.Trim();
                var view = row.DeloitteView?.Trim();

                // Skip blank rows and the description row under the header (it has no real Deloitte View).
                ...
                list.Add(new DeloitteViewRecord
                {
            ...
            _logger.LogInformation(
                "Loaded {Total} prior Deloitte View(s) across {Jurisdictions} jurisdiction(s) from '{FilePath}'.",
                total, index.Count, _options.FilePath);

            return index.ToDictionary(kvp => kvp.Key, kvp => (IReadOnlyList<DeloitteViewRecord>)kvp.Value, StringComparer.OrdinalIgnoreCase);
        }
        catch (Exception ex) when (ex is IOException or CsvHelperException)
        {
            _logger.LogWarning(ex, "Failed to read the Deloitte Views CSV at '{FilePath}'; continuing without prior-view exemplars.", _options.FilePath);
            return Empty();
        }
    }
    ...
    private static IReadOnlyDictionary<string, IReadOnlyList<DeloitteViewRecord>> Empty() =>
        new Dictionary<string, IReadOnlyList<DeloitteViewRecord>>(StringComparer.OrdinalIgnoreCase);

    /// <summary>Maps the customer CSV's columns (by header name) to the <see cref="DeloitteViewRecord"/> fields.</summary>
