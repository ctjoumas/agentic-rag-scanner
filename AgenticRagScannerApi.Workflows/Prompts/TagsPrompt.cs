...
/// <summary>
/// System- and user-prompt builders for the Tags agent (Epic 8, story 8.3). It runs ONCE per topic group
/// and selects the applicable tags across ALL the group's vetted regulatory updates (not per item). The
/// approved tag vocabulary is injected (loaded from Cosmos RegDocs at runtime) rather than hardcoded, so
/// the set can be re-seeded without a prompt change. Versioned via <see cref="Version"/> so eval runs can
...
public static class TagsPrompt
{
    ...
    public static string BuildSystemPrompt(string jurisdiction, IReadOnlyList<string> tags)
    {
        ...
        builder.AppendLine(
            ...
            "jurisdiction. Assign the applicable tags for the topic group as a whole, considering all of the " +
            "updates together.");
        builder.AppendLine();
        ...
        builder.AppendLine(
            "- This is multi-label: assign EVERY tag that genuinely applies to any of the updates (zero, one, or " +
    ...
    public static string BuildUserPrompt(TopicGroupContext context, string updatesBlock)
    {
        ...
        builder.Append("Return every applicable tag for this topic group (verbatim from the approved list) with a one-sentence rationale.");
        return builder.ToString();
    }
}
