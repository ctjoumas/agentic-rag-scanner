...
/// <summary>
/// System- and user-prompt builders for the Deloitte View agent (Epic 8, story 8.5). In a single call
/// the agent produces ONE consolidated record per topic group - both a neutral <c>SummaryOfUpdate</c> and
/// the practitioner-style <c>DeloitteView</c> - grounded on the <em>full text</em> of the group's carried
/// regulatory updates (plus their impact areas and tags). Prior Deloitte View records (retrieved by
/// jurisdiction, shaped like the historical CSV) are injected as house-style exemplars that steer the
/// DeloitteView only; the summary does not use them. Versioned via <see cref="Version"/> so eval runs can
/// attribute output changes to prompt changes (see <c>docs/prompt-management.md</c>).
/// </summary>
public static class DeloitteViewPrompt
{
    ...
    public static string BuildSystemPrompt(string jurisdiction)
    {
        ...
        builder.AppendLine(
            "You are a Deloitte employment-taxes practitioner producing a single, consolidated \"Deloitte View\" " +
        ...
        builder.AppendLine(
            "- deloitteView: concise, practical client advice - what the changes mean for employers/clients and the " +
        ...
        builder.AppendLine(
            "- Write the deloitteView in the house style and tone of the prior Deloitte View records provided as " +
        ...
        builder.AppendLine(
            ...
            "the examples steer the deloitteView only.");
        builder.AppendLine(
        ...
        builder.AppendLine(
            "- If no prior examples are provided, still write in a professional Deloitte advisory style.");
    ...
    /// carried updates (each with its full text), and the prior Deloitte View records for the jurisdiction
    ...
    public static string BuildUserPrompt(
        ...
        IReadOnlyList<DeloitteViewRecord> priorViews)
    {
        var builder = new StringBuilder();
        ...
        {
            builder.AppendLine(
                "=== Prior Deloitte View records for this jurisdiction ===\n(none available - write in a professional Deloitte advisory style)");
        }
        else
        {
            builder.AppendLine($"=== Prior Deloitte View records for {context.Run.Jurisdiction} (house-style examples; do NOT copy their facts) ===");
            var index = 1;
            foreach (var prior in priorViews)
            {
                ...
                AppendExemplarField(builder, "Deloitte View", prior.DeloitteView);
                builder.AppendLine();
            }
        }

        builder.Append("Produce the single consolidated Deloitte View record for this topic group now.");
        return builder.ToString();
    }
